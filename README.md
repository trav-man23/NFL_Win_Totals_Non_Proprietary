...
Now it gets tricky. I didn't want to just bet on a single value - a model can say '9.2 wins', but is it 9.2 wins across what distribution? Plus or minus 3 wins? To handle this uncertainty, I first used a plain Binomial model (treating the predicted win probability as known with certainty) to handle the uncertainty. But I found this would understate that uncertainty because the predictions (the 9.2) aren't perfect, thus making the resulting edge look better than it actually is. Instead, each prediction is converted into a full probability distribution using a **Beta-Binomial** model: a team's per-game win probability is itself drawn from a Beta distribution (reflecting the model's own prediction uncertainty), then a season is simulated as a Binomial draw given that probability — repeated over 10,000 Monte Carlo runs. This produces a realistic, appropriately wide distribution over possible win totals, which is what allows a real EV calculation against the sportsbook line and odds, rather than just "predicted > line."
